...
""";
    ...
    public async Task<IReadOnlyList<SyncMappingSummary>> GetMappingSummariesAsync(CancellationToken ct=default){await using var c=Open();await using var cmd=c.CreateCommand("SELECT m.mapping_id,(SELECT COUNT(*) FROM mapping_local_items i WHERE i.mapping_id=m.mapping_id AND i.item_type='File'),(SELECT COUNT(*) FROM mapping_pending_operations p WHERE p.mapping_id=m.mapping_id AND p.state <> 'Completed' AND p.state <> 'Failed' AND p.state <> 'FailedPermanent'),(SELECT COUNT(*) FROM mapping_pending_operations p WHERE p.mapping_id=m.mapping_id AND (p.state='Failed' OR p.state='FailedPermanent')) FROM sync_mappings m ORDER BY m.created_at");await using var r=await cmd.ExecuteReaderAsync(ct);var result=new List<SyncMappingSummary>();while(await r.ReadAsync(ct))result.Add(new(Guid.Parse(r.GetString(0)),r.GetInt32(1),r.GetInt32(2),r.GetInt32(3),null));return result;}
