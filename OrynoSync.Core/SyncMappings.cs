...
""";
    ...
    public async Task EnqueueAsync(MappingPendingOperation o,CancellationToken ct=default){
        ...
        await using var find=c.CreateCommand("SELECT operation_id,state FROM mapping_pending_operations WHERE mapping_id=$mapping AND relative_path=$p AND state IN ('Pending','Retrying') ORDER BY created_at DESC LIMIT 1");
    ...
    public async Task<SyncDashboardSummary> GetDashboardSummaryAsync(CancellationToken ct=default){await using var c=Open();await using var cmd=c.CreateCommand("SELECT (SELECT COUNT(*) FROM mapping_local_items WHERE item_type='File'),(SELECT COUNT(*) FROM mapping_pending_operations WHERE state <> 'Completed' AND state <> 'Failed' AND state <> 'FailedPermanent'),(SELECT COUNT(*) FROM mapping_pending_operations WHERE state='Failed' OR state='FailedPermanent'),(SELECT COUNT(*) FROM sync_mappings)");await using var r=await cmd.ExecuteReaderAsync(ct);await r.ReadAsync(ct);return new(r.GetInt32(0),r.GetInt32(1),r.GetInt32(2),r.GetInt32(3),null);}
    public async Task<IReadOnlyList<SyncMappingSummary>> GetMappingSummariesAsync(CancellationToken ct=default){await using var c=Open();await using var cmd=c.CreateCommand("SELECT m.mapping_id,(SELECT COUNT(*) FROM mapping_local_items i WHERE i.mapping_id=m.mapping_id AND i.item_type='File'),(SELECT COUNT(*) FROM mapping_pending_operations p WHERE p.mapping_id=m.mapping_id AND p.state <> 'Completed' AND p.state <> 'Failed' AND p.state <> 'FailedPermanent'),(SELECT COUNT(*) FROM mapping_pending_operations p WHERE p.mapping_id=m.mapping_id AND (p.state='Failed' OR p.state='FailedPermanent')),m.last_successful_file_sync FROM sync_mappings m ORDER BY m.created_at");await using var r=await cmd.ExecuteReaderAsync(ct);var result=new List<SyncMappingSummary>();while(await r.ReadAsync(ct))result.Add(new(Guid.Parse(r.GetString(0)),r.GetInt32(1),r.GetInt32(2),r.GetInt32(3),r.IsDBNull(4)?null:DateTimeOffset.Parse(r.GetString(4))));return result;}
    public async Task<IReadOnlyList<SyncFileError>> GetErrorsAsync(int limit=50,CancellationToken ct=default){await using var c=Open();await using var cmd=c.CreateCommand("SELECT operation_id,mapping_id,relative_path,operation_type,last_error,attempt_count,created_at FROM mapping_pending_operations WHERE state='Failed' OR state='FailedPermanent' ORDER BY created_at DESC LIMIT $limit");Add(cmd.Parameters,"$limit",Math.Clamp(limit,1,500));await using var r=await cmd.ExecuteReaderAsync(ct);var result=new List<SyncFileError>();while(await r.ReadAsync(ct)){var raw=r.IsDBNull(4)?null:r.GetString(4);var mapped=UserFacingErrorMapper.Map(raw);result.Add(new(Guid.Parse(r.GetString(0)),Guid.Parse(r.GetString(1)),r.GetString(2),r.GetString(3),mapped.Code,mapped.Message,raw,r.GetInt32(5),DateTimeOffset.Parse(r.GetString(6))));}return result;}
