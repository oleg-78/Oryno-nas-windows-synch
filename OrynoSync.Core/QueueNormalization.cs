...
public static class QueueNormalizer
{
    ...
    {
        var l = local.ToDictionary(x => PathRules.NormalizeRelative(x.RelativePath), StringComparer.OrdinalIgnoreCase); var s = server.Where(x => !x.IsDeleted).ToDictionary(x => PathRules.NormalizeRelative(x.RelativePath), StringComparer.OrdinalIgnoreCase); var ops = new List<NormalizedOperation>(); var conflicts = new List<string>();
