...
public static class QueueNormalizer
{
    public static QueueNormalizationResult Plan(IEnumerable<DesiredLocalItem> local, IEnumerable<RemoteItemState> server, IEnumerable<MappingPendingOperation> oldPending)
    {
        ...
        var l = local.Where(x => !ignore.IsIgnored(x.RelativePath)).ToDictionary(x => PathRules.NormalizeRelative(x.RelativePath), StringComparer.OrdinalIgnoreCase); var s = server.Where(x => !x.IsDeleted && !ignore.IsIgnored(x.RelativePath)).ToDictionary(x => PathRules.NormalizeRelative(x.RelativePath), StringComparer.OrdinalIgnoreCase); var ops = new List<NormalizedOperation>(); var conflicts = new List<string>();
