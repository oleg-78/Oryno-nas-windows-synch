...
public sealed class MappingCardViewModel : ViewModelBase
{
    ...
    private static string FriendlyStatus(MappingStatus status) => status switch { MappingStatus.Scanning => "Scanning...", MappingStatus.UpToDate => "Up to date", MappingStatus.WaitingForContentSupport => "Waiting for server content sync", MappingStatus.ServerRootUnavailable => "NAS folder not selected", MappingStatus.LocalFolderUnavailable => "Folder unavailable", MappingStatus.Paused => "Paused", MappingStatus.AuthenticationRequired => "Sign in required", MappingStatus.Conflict => "Conflicts need attention", MappingStatus.Error => "Unable to sync this folder", MappingStatus.Offline => "Offline - local changes are safe", _ => "Waiting to sync" };
}
