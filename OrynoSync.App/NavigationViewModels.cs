...
{
    private string _status = "Connecting to Oryno NAS...", _connectionMessage = "Starting connection check.", _connectionTone = "Warning", _indexed = "0", _waiting = "0", _errors = "0", _lastSync = "No successful file sync yet", _queue = "0 changes waiting safely", _errorSummary = "No sync errors", _historical = "No resolved history";
    private bool _syncRunning, _hasErrors, _errorsExpanded;
...
public sealed class SyncFileErrorRowViewModel(SyncFileError error) : ViewModelBase { private SyncFileError _error = error; public Guid OperationId => _error.OperationId; public string RelativePath => _error.RelativePath; public string UserMessage => _error.UserMessage; public string StateText => _error.Lifecycle switch { ErrorLifecycle.Retrying => "Retrying", ErrorLifecycle.Historical => "Resolved (history)", _ => "Active — needs attention" }; public bool IsActive => _error.Lifecycle == ErrorLifecycle.Active; public bool CanRetry => _error.Lifecycle != ErrorLifecycle.Historical; public string AttemptsText => $"Attempts: {_error.AttemptCount:N0} · Last attempt: {_error.LastAttemptAt.LocalDateTime:g}"; public Action? Retry { get; set; } = null; public void Apply(SyncFileError error) { _error = error; Raise(nameof(RelativePath)); Raise(nameof(UserMessage)); Raise(nameof(AttemptsText)); Raise(nameof(StateText)); Raise(nameof(IsActive)); Raise(nameof(CanRetry)); } }
...
{
    private SyncMapping _mapping; private string _status = "Offline", _pending = "0 changes waiting", _rootName = "NAS folder not selected", _indexed = "0 indexed", _errors = "0 errors", _lastSync = "Last sync: Never"; private bool _isScanning;
    ...
    public Action? Open { get; set; } public Action? StartSync { get; set; } public Action? StopSync { get; set; } public Action? Remove { get; set; } public Action? Rebuild { get; set; } public Action? Repair { get; set; } public Action? CreateRoot { get; set; } public Action? RefreshRoots { get; set; } public Action? ChooseDestination { get; set; }
    // Button visibility: Start visible when stopped, Stop visible when running
    public bool IsStopped => _mapping.Status is MappingStatus.Stopped or MappingStatus.ReadyForPreflight or MappingStatus.ReadyToSync;
    public bool IsSyncing => _mapping.Status is MappingStatus.Syncing or MappingStatus.Scanning;
    public bool HasDestination => _mapping.ServerRootId is not null;
    public bool CanStart => IsStopped && HasDestination;
    public bool CanStop => IsSyncing;
    public MappingCardViewModel(SyncMapping mapping) { _mapping = mapping; Apply(mapping); }
    public void Apply(SyncMapping mapping, ScanProgress? progress = null) { _mapping = mapping; Raise(nameof(Mapping)); Raise(nameof(LocalPath)); RootName = FormatRootName(mapping); var status = progress?.IsComplete == true && mapping.Status == MappingStatus.Scanning ? mapping.ServerRootId is null ? MappingStatus.ServerRootUnavailable : MappingStatus.Syncing : mapping.Status; Status = FriendlyStatus(status); IsScanning = mapping.Status == MappingStatus.Scanning || progress is not null && !progress.IsComplete; if (progress is not null) PendingText = progress.IsComplete ? $"{progress.FilesFound:N0} files indexed locally" : $"Scanning... {progress.FilesFound:N0} files found"; Raise(nameof(IsStopped)); Raise(nameof(IsSyncing)); Raise(nameof(HasDestination)); Raise(nameof(CanStart)); Raise(nameof(CanStop)); }
    ...
    private static string FriendlyStatus(MappingStatus status) => status switch { MappingStatus.Scanning => "Scanning...", MappingStatus.Syncing => "Syncing files...", MappingStatus.UpToDate => "Up to date", MappingStatus.WaitingForContentSupport => "Syncing files...", MappingStatus.ServerRootUnavailable => "Needs attention: NAS destination missing", MappingStatus.LocalFolderUnavailable => "Folder unavailable", MappingStatus.Paused => "Paused", MappingStatus.AuthenticationRequired => "Sign in required", MappingStatus.Conflict => "Conflicts need attention", MappingStatus.Error => "Unable to sync this folder", MappingStatus.Offline => "Offline - local changes are safe", MappingStatus.Stopped => "Stopped", MappingStatus.ReadyForPreflight => "Stopped", MappingStatus.ReadyToSync => "Stopped", _ => "Waiting to sync" };
}

public sealed class SettingsViewModel : ViewModelBase
{
    private string _serverUrl = "https://oryno-nas.remo78.ru", _connectionStatus = "Disconnected", _connectionTone = "Error", _device = Environment.MachineName + " - Windows", _database = "-", _watcher = "Starting...", _queue = "0", _testResult = ""; private bool _startWithWindows, _hasCredential, _isTesting, _isConnecting;
    public string ServerUrl { get => _serverUrl; set => Set(ref _serverUrl, value); } public string ConnectionStatus { get => _connectionStatus; set => Set(ref _connectionStatus, value); } public string ConnectionTone { get => _connectionTone; set => Set(ref _connectionTone, value); } public bool StartWithWindows { get => _startWithWindows; set { Set(ref _startWithWindows, value); try { WindowsAutostart.Set(value); SaveSetting?.Invoke("start_with_windows", value ? "true" : "false"); } catch (Exception ex) { DiagnosticsLogger.Write("AUTOSTART_REGISTER", $"enabled={value} outcome=failed error={ex.Message}"); } } } public string Device { get => _device; set => Set(ref _device, value); } public string DatabasePath { get => _database; set => Set(ref _database, value); } public string WatcherStatus { get => _watcher; set => Set(ref _watcher, value); } public string QueueLength { get => _queue; set => Set(ref _queue, value); } public bool HasCredential { get => _hasCredential; set => Set(ref _hasCredential, value); } public bool IsTesting { get => _isTesting; set => Set(ref _isTesting, value); } public bool IsConnecting { get => _isConnecting; set => Set(ref _isConnecting, value); } public string TestResult { get => _testResult; set => Set(ref _testResult, value); }
 /// <summary>§1: the install default is enabled — HKCU Run, per user, no elevation, no Task Scheduler.</summary>
 public void InitializeStartup() { _startWithWindows = true; Raise(nameof(StartWithWindows)); WindowsAutostart.ValidateAndRepair(); if (!WindowsAutostart.Enabled) WindowsAutostart.Set(true); }
 /// <summary>§1: re-applies a previously persisted choice without writing it back to the store.</summary>
 public void ApplyStartupSetting(bool enabled) { _startWithWindows = enabled; Raise(nameof(StartWithWindows)); try { if (enabled) { WindowsAutostart.ValidateAndRepair(); if (!WindowsAutostart.Enabled) WindowsAutostart.Set(true); } else if (WindowsAutostart.Enabled) WindowsAutostart.Set(false); } catch (Exception ex) { DiagnosticsLogger.Write("AUTOSTART_REGISTER", $"enabled={enabled} outcome=failed error={ex.Message}"); } }
 /// <summary>Persists user choices (currently the autostart toggle) in the sync database.</summary>
 public Action<string, string>? SaveSetting { get; set; }
